...
import { Bell, FileText, LockIcon, Share2, Users } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-blue-800 font-medium">Your Medical Data is Secure</h3>
            <p className="text-blue-700 text-sm mt-1">
              You have full control over who can access your medical information. 
              {accessRequests.filter(r => r.status === 'pending').length > 0 && 
                ` You have ${accessRequests.filter(r => r.status === 'pending').length} pending access requests.`}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          <dl className="divide-y divide-gray-100">
            <div className="grid grid-cols-3 gap-4 py-3">
              ...
              <dd className="col-span-2 text-gray-700">{personalInfo.name}</dd>
            </div>
            <div className="grid grid-cols-3 gap-4 py-3">
              <dt className="font-medium text-gray-900">Date of Birth</dt>
              <dd className="col-span-2 text-gray-700">{personalInfo.dateOfBirth}</dd>
            </div>
            <div className="grid grid-cols-3 gap-4 py-3">
              <dt className="font-medium text-gray-900">Blood Type</dt>
              <dd className="col-span-2 text-gray-700">{personalInfo.bloodType}</dd>
            </div>
            <div className="grid grid-cols-3 gap-4 py-3">
              <dt className="font-medium text-gray-900">Allergies</dt>
              <dd className="col-span-2 text-gray-700">
                {personalInfo.allergies.join(", ")}
              </dd>
            </div>
            <div className="grid grid-cols-3 gap-4 py-3">
              <dt className="font-medium text-gray-900">Emergency Contact</dt>
              <dd className="col-span-2 text-gray-700">{personalInfo.emergencyContact}</dd>
            </div>
          </dl>
        </div>
      </Card>
  ...
  const renderAccessRequests = () => (
    <div className="space-y-4">
      {accessRequests.map(request => (
        <Card key={request.id} className={request.status === 'pending' ? 'border border-yellow-200 bg-yellow-50' : ''}>
          <div className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Requested: {request.date}</p>
                <div className="mt-2">
                  <p className="text-sm font-medium text-gray-900">Requesting access to:</p>
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{record.provider} • {record.date}</p>
                </div>
              </div>
